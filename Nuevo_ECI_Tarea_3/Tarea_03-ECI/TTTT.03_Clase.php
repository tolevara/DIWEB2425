...
if (isset($_REQUEST['enviar'])) {
    ...
    // PREPARA LA SENTENCIA (SEGUNDO PASO)
    $sentenciaSQL = "INSERT INTO usuarios (correo, clave, nombre, autonomo, nif_cif) 
    VALUES (?,?,?,?,?)";

    $sentenciaPreparada = $conexion->prepare($sentenciaSQL); //(PRIMERA ENCRIPTACIÓN"normalita")//
    ...
    function eliminarUsuario(correo) {
        if (confirm("¿Quieres eliminar este usuario?")) {
            fetch('eliminar.php', {
                ...
                .then(response => response.json()) // RECIBE LA RESPUESTA en JSON
                .then(data => {
                    alert(data.mensaje); // MUESTRA MENSAJE
                    if (data.success) {
    ...
    <footer>
        ...
        <section aria-label="formulario">
            <form action="#" method="post">
                ...
                <fieldset> <!--AQUÍ EMPIEZA LA PARTE DE AUTÓMO-->
                    <legend>Autónomo:</legend>
