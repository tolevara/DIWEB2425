...
//EJEMPLO_12_Interfaces y Rasgos.PHP// = (/var/www/html/curso/PHP-POO = Navegador = //localhost/curso/PHP-POO

/* LAS INTERFACES Y LOS RASGOS(TRAIT) SE PONEN ANTES QUE LAS CLASES | TRAITS ES UNA INTERFAZ CON MÉTODOS 'NO' ABSTRASTO(DESARROLLADO)*/

interface iFlota {    #<-ESTA ES LA INTERFAZ#
...
trait tBonito {      #<-ESTE ES EL TRAIT# //LOS TRAIT SÓLO SE USAN EN PHP// 
...
abstract class  Camion  #DECLARACIÓN DE LA CLASE# //ESTA SERÁ LA CLASE PADRE//
{
    ...
    #UN MÉTODO ABSTRACTO *PADRE* TIENE QUE DESARROLLARSE EN LA CLASE HIJO, SI NO Explota!!#
    abstract public static function leeTren( 
        string $modelo, int $potencia, float $precio, bool $electrico, bool $remolque2 
    ) : TrenCarretera;
}

# CLASE DE LA COMPOSICIÓN (Motor)#
class Motor {            #ARTIBUTOS TIPADOS#
    public string $marca; #<-ATRIBUTO#
    public int $cilindrada;

    public function __construct(string $marca, int $cilindrada) #<-CONSTRUCTOR#
    {
        $this->marca = $marca;
        $this->cilindrada = $cilindrada;
    }

    public function __toString(): string  #<-MÉTODO toString#
    {
        return json_encode([
            'Marca' => $this->marca,
...
class TrenCarretera extends Camion implements iFlota {  #<-(implements) SE PONE PARA FORZAR LA IMPLEMENTACION DE LOS MÉTODOS ABSTRACTOS#
    use tBonito;                                     #<-AQUÍ SE PONE EL trait //SIEMPRE ANTES DE public//#
  public Motor $motorTren; #<-AÑADO EL ATRIBUTO DE LA COMPOSICIÓN#
...
if (isset($_REQUEST['enviar'])) {
    ...
    //CREAMOS EL CAMIÓN//
   /* $MiCamion = new Camion($modelo, $potencia, $precio, $electrico); <- NO SE PUEDE CREAR UNA CLASE abstract!!
 ...
 #VAMOS A CREARNOS UNA FLOTA DE TRENES DE CARRETERA# //SCRIP PRINCIPAL//
