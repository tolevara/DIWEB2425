...
require_once 'orquesta.php';
require_once 'ensayo.php';
require_once 'iconcierto.php';
...
abstract class Artista {
    ...
    (string $nombre, int $edad, bool $genero)
    {
        $this->nombre = $nombre;
        $this->setEdad($edad);
        $this->genero = $genero;
     ...
     public function setEdad (bool $edad): void {
        if($edad > 0) {
            $this->edad = $edad;
        }
     }

     public function getEdad (): bool
    {
        return $this->edad;
     }
     #AQUÍ FALTA METODO ABSTRACTO DEL PADRE
}
...
class Musico extends Artista implements iConcierto {
    ...
    public function __construct(
        ...
        $orquesta)
    {
        parent::__construct($nombre, $edad, $genero, $instrumento, $orquesta);
