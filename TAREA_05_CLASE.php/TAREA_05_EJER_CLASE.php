...
class Musico extends Artista implements iConcierto
{
    ...
    public function tocar(): void {
        ...
        $musico = new Musico("Juan", 30, true, "Guitarra");
        $mensaje = "<br>";

        $instrumentos = ["Guitarra", "Batería", "Piano", "Violín", "Bajo"];
       
        foreach ($instrumentos as $indice => $instrumento) {
            $mensaje .= "[$indice] -> $instrumento <br>";
        }

        if (in_array($musico->instrumento, $instrumentos)) {
            $mensaje .= "Tocando el instrumento";
        } else {
            $mensaje .= "No toca el instrumento";
        }
    }
